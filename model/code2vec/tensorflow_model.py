...
class Code2VecModel(Code2VecModelBase):
    def __init__(self, config: Config):
        ...
        self.eval_method_path_op, self.predict_method_path_op = None, None
        self.eval_example_loss_op, self.predict_example_loss_op = None, None
    ...
    def train(self):
        ...
        optimizer, batch_loss_op, method_path_op, \
            original_words_op, top_words_op, top_scores_op, example_loss_op = \
                self._build_tf_training_graph(input_tensors)
        ...
        try:
            log_epoch = 1  # update for load
            while True:
                # Each iteration = batch. We iterate as long as the tf iterator (reader) yields batches.
                batch_num += 1

                # Actual training for the current batch.
                _, batch_loss, method_path, original_words, top_words, top_scores, example_loss = \
                    self.sess.run([optimizer, batch_loss_op, method_path_op,
                                   original_words_op, top_words_op, top_scores_op, example_loss_op])

                method_path = common.binary_to_string_list(method_path)
                ...
                original_words = common.binary_to_string_list(original_words)  # (batch,)

                result_save_path = self.config.MODEL_SAVE_PATH.replace("models", "results")\
                                       .replace("saved_model", "") + "log_{}_ep{}.txt"
                pathlib.Path(os.path.dirname(result_save_path)).mkdir(parents=True, exist_ok=True)
                with open(result_save_path.format("train", log_epoch), 'a') as log_train_output_file:
                    self._log_prediction_results(
                        zip(method_path, original_words, top_words, top_scores, example_loss),
                        log_train_output_file
                    )
                ...
                if batch_num % num_batches_to_save_and_eval == 0:
                    ...
                    model_save_path = self.config.MODEL_SAVE_PATH + '_iter' + str(log_epoch)
                    self.save(model_save_path)
                    self.log('Saved after %d epochs in: %s' % (log_epoch, model_save_path))
                    evaluation_results = self.evaluate(result_save_path.format("test", log_epoch))
    ...
    def evaluate(self, result_save_path='log.txt') -> Optional[ModelEvaluationResults]:
        ...
        if self.eval_reader is None:
            ...
            self.eval_method_path_op, self.eval_original_names_op, self.eval_top_words_op, self.eval_top_values_op, \
            _, _, _, _, self.eval_example_loss_op = self._build_tf_test_graph(input_tensors)
        ...
        with open(result_save_path, 'w') as log_output_file:
            total_predictions = 0
            ...
            try:
                while True:
                    method_path, original_names, top_words, top_scores, example_loss = self.sess.run(
                        [self.eval_method_path_op, self.eval_original_names_op, self.eval_top_words_op,
                         self.eval_top_values_op, self.eval_example_loss_op],
                    )
                    ...
                    self._log_prediction_results(
                        zip(method_path, original_names, top_words, top_scores, example_loss),
                        log_output_file
                    )
    ...
    def _build_tf_training_graph(self, input_tensors, normalize_scores=False):
        ...
        method_path = input_tensors.method_path
        ...
        with tf.compat.v1.variable_scope('model'):
            ...
            example_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
                ...
                logits=logits)
            batch_loss = tf.reduce_sum(example_loss) / batch_size

            optimizer = tf.compat.v1.train.AdamOptimizer().minimize(batch_loss)

            topk_candidates = tf.nn.top_k(logits, k=tf.minimum(
        ...
        return optimizer, batch_loss, method_path, original_words, top_words, top_scores, example_loss
    ...
    def _build_tf_test_graph(self, input_tensors, normalize_scores=False):
        ...
        return method_path, original_words, top_words, top_scores, input_tensors.path_source_token_strings, \
               input_tensors.path_strings, input_tensors.path_target_token_strings, attention_weights, example_loss
    ...
    def predict(self, predict_data_lines: Iterable[str]) -> List[ModelPredictionResults]:
        if self.predict_reader is None:
            ...
            self.predict_method_path_op, self.predict_original_names_op, self.predict_top_words_op, self.predict_top_values_op, \
            self.predict_source_string, self.predict_path_string, self.predict_path_target_string, \
            self.attention_weights_op, self.predict_example_loss_op = \
        ...
        for line in predict_data_lines:
            method_path, batch_original_name, batch_top_words, batch_top_scores, \
                batch_path_source_strings, batch_path_strings, batch_path_target_strings, \
                batch_attention_weights, batch_example_loss = self.sess.run(
                    [self.predict_method_path_op, self.predict_original_names_op, self.predict_top_words_op, self.predict_top_values_op,
                     self.predict_source_string, self.predict_path_string, self.predict_path_target_string,
                     self.attention_weights_op, self.predict_example_loss_op],
            ...
            method_path = common.binary_to_string(method_path[0])
    ...
    def _get_vocab_embedding_as_np_array(self, vocab_type: VocabType) -> np.ndarray:
        ...
        if self.eval_reader is None:
            self.eval_reader = PathContextReader(vocabs=self.vocabs,
                                                 ...
                                                 config=self.config, estimator_action=EstimatorAction.Evaluate)
            input_iterator = tf.compat.v1.data.make_initializable_iterator(self.eval_reader.get_dataset())
            _ = self._build_tf_test_graph(input_iterator.get_next())
    ...
    def _log_prediction_results(self, results, output_file):
        for method_path, original_names, top_words, top_scores, example_loss in results:
            out_line = "{},{},{},{},{}".format(method_path, original_names, top_words[0], top_scores[0], example_loss)
...
class _TFTrainModelInputTensorsFormer(ModelInputTensorsFormer):
    def to_model_input_form(self, input_tensors: ReaderInputTensors):
        return input_tensors.method_path, input_tensors.target_string, input_tensors.target_index, \
               input_tensors.path_source_token_indices, input_tensors.path_indices, input_tensors.path_target_token_indices, \
               input_tensors.context_valid_mask

    def from_model_input_form(self, input_row) -> ReaderInputTensors:
        return ReaderInputTensors(
            method_path=input_row[0],
            target_string=input_row[1],
            target_index=input_row[2],
            path_source_token_indices=input_row[3],
            path_indices=input_row[4],
            path_target_token_indices=input_row[5],
            context_valid_mask=input_row[6]
        )
...
class _TFEvaluateModelInputTensorsFormer(ModelInputTensorsFormer):
    def to_model_input_form(self, input_tensors: ReaderInputTensors):
        return input_tensors.method_path, input_tensors.target_string, input_tensors.target_index, \
               input_tensors.path_source_token_indices, input_tensors.path_indices, input_tensors.path_target_token_indices, \
               input_tensors.context_valid_mask, \
               input_tensors.path_source_token_strings, input_tensors.path_strings, input_tensors.path_target_token_strings
    ...
    def from_model_input_form(self, input_row) -> ReaderInputTensors:
        return ReaderInputTensors(
            ...
            path_target_token_indices=input_row[5],
            context_valid_mask=input_row[6],
            path_source_token_strings=input_row[7],
            path_strings=input_row[8],
            path_target_token_strings=input_row[9]
        )
