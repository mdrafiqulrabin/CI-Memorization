...
class Model:
    ...
    def __init__(self, config):
        ...
        self.eval_predicted_indices_op, self.eval_true_target_strings_op, \
            self.eval_top_scores_op, self.eval_example_loss_op = None, None, None, None
    ...
    def train(self):
        ...
        self.queue_thread = reader.Reader(subtoken_to_index=self.subtoken_to_index,
                                          ...
                                          config=self.config)
        reader_output = self.queue_thread.get_output()
        optimizer, train_loss, predicted_indices_op, method_path_op, top_scores_op, example_loss_op = \
            self.build_training_graph(reader_output)
        true_target_strings_op = reader_output[reader.TARGET_STRING_KEY]
        ...
        for iteration in range(1, (self.config.NUM_EPOCHS // self.config.SAVE_EVERY_EPOCHS) + 1):
            ...
            result_save_path = self.config.SAVE_PATH.replace("models", "results")\
                                   .replace("saved_model", "") + "log_{}_ep{}.txt"
            pathlib.Path(os.path.dirname(result_save_path)).mkdir(parents=True, exist_ok=True)
            try:
                while True:
                    batch_num += 1
                    _, batch_loss, predicted_indices, method_path, true_target_strings, top_scores, example_loss = \
                        self.sess.run([optimizer, train_loss, predicted_indices_op,
                                       method_path_op, true_target_strings_op, top_scores_op, example_loss_op])
                    ...
                    with open(result_save_path.format("train", self.epochs_trained + 1), 'a') as log_train_output_file:
                        _ = self.update_correct_predictions(0, log_train_output_file,
                                    zip(method_path, true_target_strings, predicted_strings, top_scores, example_loss))
                ...
                results, precision, recall, f1 = \
                    self.evaluate(False, result_save_path.format("test", self.epochs_trained))
                ...
                # To save model after each epoch
    ...
    def trace(self, sum_loss, batch_num, multi_batch_start_time):
        ...
        print('Average loss at batch %d: %f, \tthroughput: %d samples/sec' % (batch_num, avg_loss,
            self.config.BATCH_SIZE * self.num_batches_to_log / (multi_batch_elapsed if multi_batch_elapsed > 0 else 1)))

    def evaluate(self, release=False, result_save_path='log.txt'):
        ...
        if self.eval_queue is None:
            ...
            self.eval_method_path_op, self.eval_predicted_indices_op, self.eval_top_scores_op, _, _, \
                self.eval_example_loss_op = self.build_test_graph(reader_output)
        ...
        with open(result_save_path, 'w') as output_file:
            ...
            try:
                while True:
                    method_path, predicted_indices, true_target_strings, top_scores, example_loss = self.sess.run(
                        [self.eval_method_path_op, self.eval_predicted_indices_op, self.eval_true_target_strings_op,
                         self.eval_top_scores_op, self.eval_example_loss_op],
                    )
                    method_path = Common.binary_to_string_list(method_path)
                    true_target_strings = Common.binary_to_string_list(true_target_strings)
    ...
    def update_correct_predictions(self, num_correct_predictions, output_file, results):
        assert self.config.BEAM_WIDTH == 0  # BEAM_WIDTH?
        for method_path, original_name, predicted, top_scores, example_loss in results:
            ...
            original_names = Common.internal_delimiter.join(filtered_original)

            filtered_predicted_first_parts = Common.filter_impossible_names(predicted)  # list
            top_words = Common.internal_delimiter.join(filtered_predicted_first_parts)

            score_first = [val[0] for val in top_scores[:len(filtered_predicted_first_parts)]]
            loss_first = [val for val in example_loss[:len(filtered_predicted_first_parts)]]

            avg_score, avg_loss = 0, 0
            if len(score_first) > 0:
                avg_score = np.mean(score_first)
            if len(loss_first) > 0:
                avg_loss = np.mean(loss_first)

            out_line = "{},{},{},{},{}".format(method_path, original_names, top_words, avg_score, avg_loss)
    ...
    def build_training_graph(self, input_tensors):
        method_path = input_tensors[reader.METHOD_PATH_KEY]
        ...
        with tf.variable_scope('model'):
            ...
            example_loss = crossent * target_words_nonzero
            loss = tf.reduce_sum(example_loss) / tf.to_float(batch_size)
        ...
        return train_op, loss, predicted_indices, method_path, topk_values, example_loss
    ...
    def build_test_graph(self, input_tensors):
        ...
        return method_path, predicted_indices, topk_values, target_index, attention_weights, example_loss
    ...
    def predict(self, predict_data_lines):
        if self.predict_queue is None:
            ...
            _, self.predict_top_indices_op, self.predict_top_scores_op, _, self.attention_weights_op, _ = \
                self.build_test_graph(reader_output)
            self.predict_method_path_op = reader_output[reader.METHOD_PATH_KEY]
        ...
        for line in predict_data_lines:
            method_path, predicted_indices, top_scores, true_target_strings, attention_weights, path_source_string, path_strings, path_target_string = self.sess.run(
                [self.predict_method_path_op, self.predict_top_indices_op, self.predict_top_scores_op, self.predict_target_strings_op,
            ...
            results.append((method_path, true_target_strings, predicted_strings, top_scores, attention_per_path))
        return results
