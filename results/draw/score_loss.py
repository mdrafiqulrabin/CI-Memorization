...
def draw_score_loss(values_t, metric_t, task_t, type_t, model_t, dataset_t, noise_t):
    plt.figure()

    title_t, xlabel_t, ylabel_t = cf.plt_get_title_labels(metric_t, noise_t, model_t, dataset_t, type_t)
    plt.title(title_t)
    plt.xlabel(xlabel_t, fontsize=18, labelpad=10)
    plt.ylabel(ylabel_t, fontsize=18, labelpad=10)

    x_len, y_len = len(values_t[0]) if metric_t == "prediction_score" else cf.max_epoch[dataset_t], 1.0
    x_ranges = [1] + cf.get_x_ranges(metric_t, task_t, dataset_t, noise_t, x_len) + [x_len]
    ...
    eval_t = "train" if metric_t == "gini_loss" else "eval"
    plot_file = cf.get_ext_filename("pdf", metric_t, task_t, type_t, noise_t, model_t, dataset_t, eval_t)
...
for metric_n in ["prediction_score", "gini_loss"]:
    for task_n in cf.subject_tasks:
        for noise_n in cf.noise_names[task_n]:
            print("\t{} - {} - {}:".format(metric_n.upper(), task_n.upper(), noise_n))
            for model_n in cf.model_names[task_n]:
                for dataset_n in cf.dataset_names[task_n]:
                    for type_n in cf.subject_tasks[task_n]:

                        eval_n = "train" if metric_n == "gini_loss" else "eval"
                        csv_file = cf.get_ext_filename("csv", metric_n, task_n, type_n,
                                                       noise_n, model_n, dataset_n, eval_n)
                        ...
                        draw_score_loss(values_n, metric_n, task_n, type_n, model_n, dataset_n, noise_n)
